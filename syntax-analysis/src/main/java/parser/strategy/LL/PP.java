...
import parser.strategy.LL.data.LLData;
import parser.strategy.LL.exceptions.LLException;
import parser.strategy.LL.structure.table.LLTable;
...
public class PP extends ParseStrategy {

    // Logger
    private Logger l = LogManager.getFormatterLogger(getClass());

    // Components
    private LLTable llppTable;

    // Root of parser
    private NonTerminalToken treeRoot;

    // Data storage
    private LLData llppData;

    public PP(Grammar grammar) {
        super(grammar);
        ...
        for(int phase = 1; phase <= phases; phase++) {
            ...
            while(!(stackSyntax.peek() instanceof EndOfStackToken)) {
                ...
                if(syntaxToken instanceof LLActionToken) {
                    ...
                    // Get cell
                    LLAbstractTableCell cell = llppTable.getCell(syntaxToken, lexicalToken);

                    // Cast to non-terminal
                    NonTerminalToken nonTerminalToken = (NonTerminalToken) syntaxToken;

                    // If a rule cell
                    if(cell instanceof LLRuleCell) {

                        // Store production
                        List<AbstractSyntaxToken> ruleCopy = ((LLRuleCell) cell).getRuleCopy();
                    ...
                    } else if(cell instanceof LLErrorCell) {

                        // Cast to error cell
                        LLErrorCell errorCell = (LLErrorCell) cell;
                        ...
                        switch (errorCell.getDecision()) {
                            case LLErrorCell.POP:

                                // Pop the data
                                stackSyntax.pop();

                                break;
                            case LLErrorCell.SCAN:
